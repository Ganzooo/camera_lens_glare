...
class vdsr(nn.Module):
    def __init__(self, vdsr_depth):
        super(vdsr, self).__init__()
        self.residual_layer = self.make_layer(Conv_ReLU_Block, vdsr_depth)
...
class UNetWithResnet50EncoderDUA_vdsr(nn.Module):
    def __init__(self, vdsr_depth):
        super().__init__()
        self.resnet_unet_dua = UNetWithResnet50EncoderDUA()
        self.vdsr = vdsr(vdsr_depth)

    def forward(self, x, with_output_feature_map=False):
        org_x = x
        x = self.resnet_unet_dua(x)
        x = self.vdsr(x, org_x)
        return x
