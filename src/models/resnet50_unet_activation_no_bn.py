...
class ConvBlock(nn.Module):
    """
    Helper module that consists of a Conv -> BN -> Drop -> ReLU
    """
...
class UNetWithResnet50Encoder_act_no_bn(nn.Module):
    DEPTH = 6
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        for i, block in enumerate(self.down_blocks, 2):
            x = block(x)
            if i == (UNetWithResnet50Encoder_act_no_bn.DEPTH - 1):
                continue
        ...
        for i, block in enumerate(self.up_blocks, 1):
            key = f"layer_{UNetWithResnet50Encoder_act_no_bn.DEPTH - 1 - i}"
