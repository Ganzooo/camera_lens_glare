...
class OutConvBlock(nn.Module):
    """
    Helper module that consists of a Conv -> BN ->  Sigmoid
    """
...
class UNetWithResnet50EncoderDUA_mish(nn.Module):
    DEPTH = 6
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        for i, block in enumerate(self.down_blocks, 2):
            x = block(x)
            if i == (UNetWithResnet50EncoderDUA_mish.DEPTH - 1):
                continue
        ...
        for i, block in enumerate(self.up_blocks, 1):
            key = f"layer_{UNetWithResnet50EncoderDUA_mish.DEPTH - 1 - i}"
            if i <=3:
