...
#parser.add_argument('--src_dir', default='/dataset_sub/camera_light_glare/train/', type=str, help='Directory for full resolution images')
#parser.add_argument('--tar_dir', default='/dataset_sub/camera_light_glare/patches_256_std/train_patch/',type=str, help='Directory for image patches')
...
parser.add_argument('--tar_dir', default='/dataset_sub/camera_light_glare/patches_256_std/val_patch/',type=str, help='Directory for image patches')
...
def save_files_random(i):
    ...
    for j in range(NUM_PATCHES):
        ...
        cv2.imwrite(os.path.join(noisy_patchDir, '{}_{}_{.2f}.png'.format(i+1,j+1)), noisy_patch,std_noisy)
        cv2.imwrite(os.path.join(clean_patchDir, '{}_{}_{.2f}.png'.format(i+1,j+1)), clean_patch,std_clean)
...
def save_files_stride(i):
    ...
    for top in range(0, noisy_img.shape[0], STRIDE):
        for left in range(0, noisy_img.shape[1], STRIDE):
            ...
            if noisy_patch.shape[0] >= PS and noisy_patch.shape[1] >= PS and clean_patch.shape[0] >= PS and clean_patch.shape[1] >= PS and std_noisy >= 12.0:
                cv2.imwrite(os.path.join(noisy_patchDir, '{}_{}.png'.format(i,top)), noisy_patch)
                cv2.imwrite(os.path.join(clean_patchDir, '{}_{}.png'.format(i,top)), clean_patch)
